  ...
  return (
    <main className="flex-1 space-y-12 pb-24">
      <section className="container space-y-6 py-20">
        ...
        <h1 className="font-serif text-4xl leading-tight text-foreground md:text-5xl">{page?.title ?? "Политика конфиденциальности"}</h1>
        <p className="max-w-3xl text-lg text-muted-foreground">
          Этот документ описывает правила обработки и хранения персональных данных клиентов студии PM BY MADI и может быть обновлён в любой момент.
        </p>
      </section>
      <section className="container prose prose-neutral max-w-none text-base leading-relaxed text-foreground">
        {paragraphs.length ? paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>) : <p>Текст будет предоставлен позже.</p>}
      </section>
    </main>
  );
}
